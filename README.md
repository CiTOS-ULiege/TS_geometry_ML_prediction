...
- `reaction_B_machine_learning_model.py`: fits Reaction B Ridge regression models and evaluates descriptors using leave-one-out cross-validation (LOOCV).
...
- `reaction_C_machine_learning_model.py`: fits Reaction C Ridge regression models and evaluates descriptors using LOOCV.
- `reaction_C_method_comparison.py`: compares regression methods for Reaction C.
...
2. Run the model-development script for Reaction B or Reaction C.
3. Run the method-comparison script to compare the available regression approaches, if required.
4. Run the external-test script to evaluate the fixed equations on an independent test set.
5. Inspect the generated metrics, predictions, equations, and parity plots.
...
```bash
python reaction_B_machine_learning_model.py --input reaction_B_dataset.csv --output reaction_B_ml_results --alpha 0.1
python reaction_B_method_comparison.py --input reaction_B_dataset.csv --output reaction_B_method_comparison --descriptor "σp+"
python reaction_B_external_test_prediction.py --input reaction_B_external-set.csv

python reaction_C_machine_learning_model.py --input reaction_C_dataset.csv --output reaction_C_ml_results --alpha 0.1
python reaction_C_method_comparison.py --input reaction_C_dataset.csv --output reaction_C_method_comparison --descriptor "σp+"
python reaction_C_external_test_prediction.py --input reaction_C_external-set.csv
```
