#!/usr/bin/env python3
"""Hammett-focused ridge models for Reaction C TS-geometry prediction.

Unlike reaction_C_descriptor_screening.py, which compares a broad set of
nucleophile descriptors (Hammett-related and DFT-derived) and ranks them by
dihedral LOOCV Q², this script builds the final Hammett-based models used for
prediction. By default it evaluates Hammett constants (σp+, σp, σp−); other
descriptor columns can be supplied with --descriptors.
...
dihedral. Models are fitted with Ridge regression and evaluated by leave-one-
out cross-validation (LOOCV). In addition to LOOCV metrics, this script
exports model equations, per-sample predictions, best-model parity plots, and
software versions for downstream analysis (for example external validation).

Example
-------
python reaction_C_machine_learning_model.py \
  --input reaction_C_dataset.csv \
  --output reaction_C_ml_results --alpha 0.1
"""
...
TARGETS = {
    "bond": ("reaA_bond", "reaC_bond", "Bond length", "Å", "%.2f"),
    "angle": ("reaA_angle", "reaC_angle", "Angle", "°", "%.1f"),
    "dihedral": ("reaA_dihedral", "reaC_dihedral", "Dihedral", "°", "%.1f"),
}
...
def arguments():
    p = argparse.ArgumentParser(
        description=(
            "Reaction C Hammett-based ridge models: fit LOOCV models, export "
            "equations, and plot best-model parity (see module docstring)."
        )
    )
    p.add_argument("--input", required=True, type=Path, help="Input CSV file.")
    p.add_argument("--output", type=Path, default=Path("reaction_C_ml_results"))
...
def load_data(path, requested):
    ...
    geometry = [c for c in data if c.startswith(("reaA_", "reaC_"))]
...
def write_equations(metrics, path):
    lines = ["Reaction C ridge-regression models", "y = a*(Reaction A geometry) + b*(descriptor) + c", ""]
