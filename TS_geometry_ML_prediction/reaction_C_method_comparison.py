#!/usr/bin/env python3
"""Compare linear regression methods for Reaction C TS-geometry prediction.

Each model uses the corresponding Reaction A geometry and one numerical
chemical descriptor to predict a Reaction C bond length, angle, or dihedral.
...
Example
-------
python reaction_C_method_comparison.py \
  --input reaction_C_dataset.csv \
  --output reaction_C_method_comparison --descriptor "σp+"
"""
...
TARGETS = {
    "bond": ("reaA_bond", "reaC_bond", "Bond length", "Å"),
    "angle": ("reaA_angle", "reaC_angle", "Angle", "°"),
    "dihedral": ("reaA_dihedral", "reaC_dihedral", "Dihedral", "°"),
}
METHODS = {
...
def parse_args():
    p = argparse.ArgumentParser(description="Compare regression methods for Reaction C.")
    p.add_argument("--input", required=True, type=Path, help="Input CSV file.")
    p.add_argument("--output", type=Path, default=Path("reaction_C_method_comparison"))
...
def make_summary(metrics):
    ...
    summary = q.rename(columns={t: f"loocv_q2_{t}" for t in TARGETS})
    summary["mean_loocv_q2_all_targets"] = q.mean(axis=1)
    return summary.reset_index().sort_values("loocv_q2_dihedral", ascending=False)
...
def save_plots(metrics, summary, out, dpi):
    ...
    ranked = summary.sort_values("loocv_q2_dihedral")
    fig, ax = plt.subplots(figsize=(7, 4.5), constrained_layout=True)
    ax.barh(ranked["method"], ranked["loocv_q2_dihedral"])
    ax.axvline(0, color="black", lw=0.8)
    ax.set_xlabel("Dihedral LOOCV Q²")
    ax.set_title("Reaction C method ranking")
...
def main():
    ...
    print(f"Highest-ranked method by dihedral LOOCV Q²: {summary.iloc[0]['method']}")
