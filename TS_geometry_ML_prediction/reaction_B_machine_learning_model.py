#!/usr/bin/env python3
"""Hammett-focused ridge models for Reaction B TS-geometry prediction.

Unlike reaction_B_descriptor_screening.py, which compares a broad set of
substituent descriptors (Hammett-related and DFT-derived) and ranks them by
bond-length LOOCV Q², this script builds the final Hammett-based models used
for prediction. By default it evaluates Hammett constants (σp+, σp, σp−); other
...
geometry with that descriptor to predict Reaction B bond length, angle, and
...
def arguments():
    p = argparse.ArgumentParser(
        description=(
            "Reaction B Hammett-based ridge models: fit LOOCV models, export "
