#!/usr/bin/env python3
"""Screen descriptors for Reaction B transition-state geometry models.

This script compares candidate substituent descriptors for Reaction B TS-geometry
...
Descriptors are ranked by bond-length LOOCV Q².
"""
...
TARGETS = {
    "bond": ("reaA_bond", "reaB_bond"),
    "angle": ("reaA_angle", "reaB_angle"),
    "dihedral": ("reaA_dihedral", "reaB_dihedral"),
}
RANKING_TARGET = "bond"
RANKING_LABEL = r"LOOCV $Q^2$ (bond length)"
FIGURE_TITLE = "Reaction B descriptor ranking"
DEFAULT_INPUT = Path(__file__).resolve().parent / "datasets/reaction_B_dataset.csv"
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "results/reaction_B_descriptor_screening"
...
def load_data(path, requested):
    ...
    geometry = [column for column in data if column.startswith(("reaA_", "reaB_"))]
